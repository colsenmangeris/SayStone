...
nonisolated enum MAITranscriptionRequest {
    ...
    static func make(samples: [Float], resource: String, key: String, boundary: String = UUID().uuidString) throws -> URLRequest {
        guard !key.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { throw failure("Add your Azure Speech key in Voice Engine settings.") }
        ...
        let definition = #"{"enhancedMode":{"enabled":true,"model":"MAI-Transcribe-2","modelOptions":{"transcribeStyle":"verbatim"}}}"#
    ...
    static func text(from data: Data, status: Int) throws -> String {
        guard (200..<300).contains(status) else {
            switch status {
            case 401, 403: throw failure("Azure rejected the key or resource access. Check your Speech resource configuration.")
            case 429: throw failure("Azure's transcription quota is busy. Please try again later.")
            default: throw failure("Azure transcription failed (HTTP \(status)).")
            }
        }
